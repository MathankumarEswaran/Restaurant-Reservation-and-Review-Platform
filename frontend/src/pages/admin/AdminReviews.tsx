...
export function AdminReviews() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-slate-500">Moderate reviews submitted across the platform.</p>

      <div className="mt-6 space-y-4">
        {reviews.map((review) => {
          const restaurant = getRestaurantById(review.restaurantId);
          return (
            <div key={review.id} className="flex items-start gap-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              ...
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500 cursor-pointer"
