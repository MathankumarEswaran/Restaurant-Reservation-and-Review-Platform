...
export function CustomerHome() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-secondary">Welcome back, {user?.name.split(" ")[0] ?? "Guest"} 👋</h1>
      ...
      <div className="mt-8 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm sm:p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-semibold text-secondary">Upcoming Reservations</h2>
          <Link to="/dashboard/reservations" className="flex items-center gap-1 text-sm font-medium text-primary">
            View all <FiArrowRight size={14} />
          </Link>
        </div>
        {upcoming.length === 0 ? (
          <p className="py-6 text-center text-sm text-slate-500">No upcoming reservations yet.</p>
        ) : (
          <div className="space-y-3">
            {upcoming.slice(0, 3).map((r) => (
              <div key={r.id} className="flex items-center gap-4 rounded-xl border border-slate-100 p-3">
                <img src={r.restaurantImage} alt={r.restaurantName} className="h-14 w-14 rounded-lg object-cover" />
                <div className="flex-1">
                  <p className="font-medium text-secondary">{r.restaurantName}</p>
      ...
      <div className="mt-8 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm sm:p-6">
        <h2 className="mb-4 font-semibold text-secondary">Favorite Restaurants</h2>
        {favorites.length === 0 ? (
          <p className="py-6 text-center text-sm text-slate-500">You haven't added any favorites yet.</p>
        ) : (
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {favorites.slice(0, 4).map((r) => (
              <Link key={r.id} to={`/restaurants/${r.slug}`} className="flex items-center gap-3 rounded-xl border border-slate-100 p-3 hover:border-primary">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{r.location.city}</p>
                </div>
              </Link>
            ))}
          </div>
